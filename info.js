/**
 * [0] => Version and EC Level
 * [1] => Total Number of Data Codewords for this Version and EC Level
 * [2] => EC Codewords Per Block
 * [3] => Number of Blocks in Group 1
 * [4] => Number of Data Codewords in Each of Group 1's Blocks
 * [5] => Number of Blocks in Group 2
 * [6] => Number of Data Codewords in Each of Group 2's Blocks
 */
let tb = [
    ["1-L", 19, 7, 1, 19, 0, 0],
    ["1-M", 16, 10, 1, 16, 0, 0],
...
/**
 * 取得指定版本與容錯等級的資訊
 * @param {number} version 版本 1~41
 * @param {string} ecLevel 容錯等級 'L', 'M', 'Q', 'H'
 * @returns {Info}
 */
function getVersionInfo(version, ecLevel) {
    if (dict[ecLevel] === undefined || dict[ecLevel][version] === undefined) {
        throw `bad (version, ecLevel) = (${version}, ${ecLevel})`;
    }
    return dict[ecLevel][version];
}

/**
 * 取得足以容納訊息最低版本的資訊
 * @param {number} len 資料長度(bits)
 * @param {?string} ecLevel 容錯等級 'L', 'M', 'Q', 'H'
 * @returns {Info}
 */
function getMinVersionInfo(len, ecLevel) {
    ecLevel = ecLevel || 'M';
    if (dict[ecLevel] === undefined) {
        throw `bad ecLevel: ${ecLevel}`;
    }
    let version = dict[ecLevel].findIndex(row => row && row.totalDataWords * 8 >= len);
    if (version < 0) {
        throw `找不到`;
    }
    return dict[ecLevel][version];
...
export { getVersionInfo, getMinVersionInfo };
